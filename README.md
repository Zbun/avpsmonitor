...
1. **Agent 上报** → VPS 上的 Agent 每 5 秒向 `/api/report` 发送系统数据
2. **IP 定位** → 服务端自动根据 IP 获取地理位置（使用 ip-api.com）
3. **数据存储** → 数据存入 Redis，设置 60 秒过期（用于判断离线）
4. **前端展示** → 前端每 5 秒从 `/api/nodes` 获取最新数据
...
| 变量名 | 必填 | 说明 | 示例 |
|--------|------|------|------|
| `API_TOKEN` | ✅ | Agent 认证 Token | `your-secret-token` |
| `VITE_USE_REAL_API` | ✅ | 使用真实 API | `true` |
...
通过 `VPS_SERVERS` 环境变量可以预先配置服务器的显示名称、国家代码、位置、**到期时间**和**流量重置日**，**优先级高于 IP 自动识别和 Agent 上报**。

**格式**：`节点ID:显示名称:国家代码:位置:到期日期:流量重置日`，多个服务器用逗号分隔

**示例**：
```
VPS_SERVERS=hk-01:香港 CN2 GIA:HK:Hong Kong:2025-12-31:1,jp-01:东京 Lite:JP:Tokyo:2025-06-15:15,us-01:洛杉矶 9929:US:Los Angeles:2026-01-01:8
```
...
> - 到期日期和流量重置日可以为空，系统会使用 Agent 上报的值或默认值
> - 每个 VPS 的流量重置日可以不同，对应各商家的计费周期
