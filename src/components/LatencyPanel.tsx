...
export const LatencyPanel: React.FC<LatencyPanelProps> = ({
  ...
  return (
    <div className="bg-white/90 dark:bg-slate-800/80 backdrop-blur-sm rounded-xl border border-slate-200 dark:border-slate-700/50 p-5">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <Zap className="w-5 h-5 text-yellow-500 dark:text-yellow-400" />
          <h2 className="text-lg font-semibold text-slate-800 dark:text-white">三网延迟测试</h2>
        </div>
        <button
          onClick={onRunTest}
          disabled={isTesting}
      ...
      <div className="flex gap-4 mb-4">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs text-slate-500 dark:text-gray-400">电信 (CT)</span>
        </div>
        <div className="flex items-center gap-1.5">
          <div className="w-3 h-3 rounded-full bg-red-500" />
          <span className="text-xs text-slate-500 dark:text-gray-400">联通 (CU)</span>
        </div>
        <div className="flex items-center gap-1.5">
          <div className="w-3 h-3 rounded-full bg-green-500" />
          <span className="text-xs text-slate-500 dark:text-gray-400">移动 (CM)</span>
        </div>
      </div>
            ...
            {nodes.map(node => {
              ...
              return (
                <tr key={node.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                  <td className="py-3">
                    <div className="flex items-center gap-2">
                      <span className={`w-2 h-2 rounded-full ${node.status === 'online' ? 'bg-green-500' :
                        node.status === 'warning' ? 'bg-yellow-500' : 'bg-red-500'
                        }`} />
                      <span className="text-sm text-slate-700 dark:text-white font-medium">{node.name}</span>
                    </div>
                  </td>
                  {['CT', 'CU', 'CM'].map(code => {
                    ...
                    return (
                      <td key={code} className="py-3 text-center">
                        ...
                        `}>
                          <span className={`text-sm font-medium ${isp ? getLatencyColor(isp.status) : 'text-gray-500'
                            }`}>
